...
| `STRIPE_PRICE_ID_OWNER` | `price_...` | Stripe → Products → GrowthOS Owner → the recurring price |
...
1. **Create the Owner product**
   ...
   - Name: `GrowthOS — Owner`
   - Pricing: Recurring, $49/month USD (or whatever you settle on)
   - Copy the price ID (`price_...`) into `STRIPE_PRICE_ID_OWNER`
...
# Deploy all three functions. stripe-webhook MUST deploy with --no-verify-jwt
# because Stripe doesn't know about Supabase JWTs — the signature check
# inside the function IS the auth. The other two use the standard JWT gate.
